...
def get_event_order(order_id, order):
    return  {
        ...
        'closeTime': order['close_time'].replace('.', '-') if 'close_time' in order else None,
        'closePrice': order['close_price'] if 'close_price' in order else None,
        'size': order['lots'],
        'profit': order['pnl'],
        'balance': order['balance'] if 'balance' in order else None,
...
class mt_connector_client():
    ...
    def check_open_orders(self):
        def get_modified_fields(ord1, ord2):
            modified_fields = []
            fields_to_check = ['lots', 'SL', 'TP']
            field_modified = lambda field: ord1[field] != ord2[field]
            for field in fields_to_check:
                if field_modified(field):
                    modified_fields.append(field)
            return modified_fields if len(modified_fields) else []
