...
def save_all_strategy_data (content):
  ...
  def save_all_strategies(strategies, results):
    ...
    return results
  def save_all_accounts(accounts, results):
    for a in accounts:
    ...
    return results
  def save_all_trades(trades, results):
    for t in trades:
      trade = Trade(
        t['orderId'],
        t['masterOrderId'],
    ...
    return results
  def save_all_orders(orders, results):
    for o in orders:
      order = Order(
        o['orderId'],
        o['masterOrderId'],
        ...
        o['openPrice'],
        o['size'],
        o['comment'] if 'comment' in o else None,
        o['sl'] if 'sl' in o else None,
        o['tp'] if 'tp' in o else None,
        o['status']
      )
      try:
        res = save_order(order)
