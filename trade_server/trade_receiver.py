...
class TradeReceiver():
  ...
  def on_historic_trades (self):
    trades = self.connector.historic_trades
    print('[RECEIVER] on_historic_trades')
    log(f"[RECEIVER] on_historic_trades")
...
def handle_order_modified (trade_queue, mt_order, modified_fields):
  ...
  print('[RECEIVER] ORDER MODIFIED:', mt_order, 'MODIFIED FIELDS:', modified_fields)
