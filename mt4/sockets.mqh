...
bool ClientSocket::Send(string strMsg)
{
   ...
   /* string msgLen = (string)StringLen(strMsg);
   while (StringLen(msgLen) < 4) {
      msgLen = (string)0 + msgLen;
   }
   strMsg = msgLen + strMsg;
      Print("Sent this message: " + strMsg);*/
...
string ClientSocket::ReceiveWithHeader()
{
   ...
   if (TerminalInfoInteger(TERMINAL_X64)) {
      ...
      int res = 1;
      while (res > 0) {
         res = recv(mSocket32, arrBuffer, ReceiveBufferSize, 0); // res = num of chars read from socket
         if (res > 0) {
            StringAdd(mPendingReceiveData, CharArrayToString(arrBuffer, 0, res));
   ...
   if (mPendingReceiveData == "") {
      ...
      int msgLength = (int)StringSubstr(mPendingReceiveData, 0, 4);
      mPendingReceiveData = StringSubstr(mPendingReceiveData, 4);
      strRetval = StringSubstr(mPendingReceiveData, 0, msgLength);
      mPendingReceiveData = StringSubstr(mPendingReceiveData, msgLength);
   }
...
string ClientSocket::Receive(string MessageSeparator = "")
{
   ...
   // Print("Pending data: " + mPendingReceiveData);
