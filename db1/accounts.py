...
from db1.common import mutate_many, mutate_one, query_many, query_one, values_placeholder
...
def get_accounts ():
  sql = "SELECT accountId, accountNumber, accountType, username, subscriptionKey, strftime('%Y-%m-%d %H:%i:%S',startDate) as startDate, strftime('%Y-%m-%d %H:%i:%S',endDate) as endDate, deposit, annualPctRet, maxDD, maxPctDD, annPctRetVsDdPct, winPct, profitFactor, numTrades FROM Accounts"
