...
from db1.common import query_many, query_one, mutate_one, values_placeholder, datetime_fmt
from db1.trades import get_trades_of_account
...
def get_strategy_info():
  sql = """
            SELECT Strategies.magic, Strategies.symbols, Strategies.timeframes, cast(Accounts.startDate as text) as startDate, cast(Accounts.endDate as text) as endDate
            FROM Strategies
            INNER JOIN Accounts ON Accounts.accountNumber = Strategies.magic
            WHERE Accounts.accountType = 'strategy_backtest'
        """
  return query_many(sql)
