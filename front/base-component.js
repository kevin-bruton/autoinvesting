export class BaseComponent extends window.HTMLElement {
  ...
  async httpPostTrades (file, strategyName, symbol, environment, platform) {
    const jwt = window.sessionStorage.getItem('t')
    const formData = new window.FormData()
    ...
    formData.append('file', file)
    const res = await window.fetch('/api/trades', { method: 'POST', headers: { Authorization: `Bearer ${jwt}` }, body: formData })
    if (res.status >= 200 && res.status <= 209) {
      const result = res.json()
      return result
    }
